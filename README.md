...
*If you pick an aggregate duplicate policy (max|mean|median|first|sum) as default --dup-policy, the script collapses duplicates before building the Plotly figure; overlay/stack keep all rows and handle layering in the client.*
...
def build_figure_payload(
    ...
    # Make JSON-safe rows (no NaN/Infinity)

    df_jsonsafe = df.replace([np.inf, -np.inf], np.nan).where(pd.notna(df), None)
    payload = {
        "rows": df_jsonsafe.to_dict(orient="records"),
...
(function() {
  try {
    ...
    (function(){
      ...
      // ---- Create/insert subplot container (CLI-driven placement) ----
      (function () {
        if (!UI || !UI.pt || !UI.pt.enable) return;

        // Resolve anchor element from selector (falls back to #rowDetails)
        var anchorSel = (UI.pt.anchor && typeof UI.pt.anchor === 'string') ? UI.pt.anchor : '#rowDetails';
        var anchor = document.querySelector(anchorSel) || document.getElementById('rowDetails');
        if (!anchor) return;

        var mode = (UI.pt.mode === 'absolute' || UI.pt.mode === 'fixed') ? UI.pt.mode : 'flow';
        var position = (UI.pt.position === 'before' || UI.pt.position === 'inside') ? UI.pt.position : 'after';
        var containerId = UI.pt.containerId || 'present-tissues-plot';

        // Reuse or create the subplot container
        var subplotCol = document.getElementById(containerId);
        if (subplotCol) subplotCol.remove();
        subplotCol = document.createElement('div');
        subplotCol.id = containerId;

        // Base styling
        var minW = (typeof UI.pt.minWidth === 'number') ? UI.pt.minWidth : 420;
        subplotCol.style.minWidth = String(minW) + 'px';
        subplotCol.style.overflowX = 'auto';
        subplotCol.style.padding = '4px 0';
        subplotCol.style.border = '1px solid #eee';
        subplotCol.style.borderRadius = '6px';
        subplotCol.style.background = 'white';
        ...
        if (UI.pt && UI.pt.title) {
          ...
          subplotCol.appendChild(hdr);
        }

        var dx = Number(UI.pt.offsetX || 0);
        var dy = Number(UI.pt.offsetY || 0);

        if (mode === 'flow') {
          // Side-by-side: wrap the anchor and append the subplot
          ...
          // Left = anchor area (e.g., details)
          anchor.style.flex = '1 1 auto';
          anchor.style.minWidth = '320px';
          anchor.style.overflow = 'auto';

          // Right = subplot column
          subplotCol.style.maxWidth = '50vw';

          // Order relative to anchor (before/after/inside)
          if (position === 'before') {
            wrap.appendChild(subplotCol);
            wrap.appendChild(anchor);
          } else if (position === 'inside') {
            // unlikely for flow, but supported
            wrap.appendChild(anchor);
            anchor.appendChild(subplotCol);
          } else {
            wrap.appendChild(anchor);
            wrap.appendChild(subplotCol);
          }

          // Visual nudge within flow
          if (dx || dy) {
            subplotCol.style.transform = 'translate(' + dx + 'px,' + dy + 'px)';
          }

        } else if (mode === 'absolute') {
          // Absolute: create a positioned wrapper next to (or inside) the anchor
          var absWrap = document.createElement('div');
          absWrap.style.position = 'relative';
          absWrap.style.minHeight = '20px';

          if (position === 'before') {
            anchor.parentNode.insertBefore(absWrap, anchor);
            absWrap.appendChild(anchor);
          } else if (position === 'after') {
            if (anchor.nextSibling) {
              anchor.parentNode.insertBefore(absWrap, anchor.nextSibling);
            } else {
              anchor.parentNode.appendChild(absWrap);
            }
          } else {
            anchor.appendChild(absWrap);
          }

          subplotCol.style.position = 'absolute';
          subplotCol.style.left = '0px';
          subplotCol.style.top = '0px';
          if (dx || dy) {
            subplotCol.style.transform = 'translate(' + dx + 'px,' + dy + 'px)';
          }
          absWrap.appendChild(subplotCol);

        } else {
          // mode === 'fixed'  (viewport fixed)
          // Create a container at the end of <body> and fix the subplot to the viewport
          var body = document.body;
          var fixedWrap = document.createElement('div');
          fixedWrap.style.position = 'fixed';
          fixedWrap.style.left = '0px';
          fixedWrap.style.top = '0px';
          fixedWrap.style.zIndex = '1000';
          if (dx || dy) {
            fixedWrap.style.transform = 'translate(' + dx + 'px,' + dy + 'px)';
          }
          fixedWrap.appendChild(subplotCol);
...
def save_html(fig: Figure, payload: Dict, ui_cfg: Dict, out_path: str, self_contained: bool, lang: str = "en"):
    ...
    import math
    def _clean(o):
        # Deep-sanitize: NaN/±Inf -> None; convert numpy scalars -> Python types
    ...
    obj = {**payload, "ui_cfg": ui_cfg}
    try:
        payload_json = json.dumps(obj, ensure_ascii=False, allow_nan=False)
    except ValueError:
        # Belt & suspenders: deep-clean and retry
        payload_json = json.dumps(_clean(obj), ensure_ascii=False, allow_nan=False)
...
def main():
    ...
    ap.add_argument("--pt-mode", choices=["flow","absolute","fixed"], default="flow",
                    help="Placement mode: 'flow' = side-by-side layout; 'absolute' = positioned relative to an anchor; 'fixed' = fixed to the viewport")
    ap.add_argument("--pt-anchor", default="#rowDetails",
                    help="CSS selector to anchor the subplot near (e.g., '#rowDetails', '#controls')")
    ap.add_argument("--pt-position", choices=["before","after","inside"], default="after",
                    help="Where to place the subplot relative to the anchor (flow/absolute modes)")
    ap.add_argument("--pt-offset-x", type=int, default=0,
                    help="Horizontal offset in pixels (positive=right, negative=left)")
    ap.add_argument("--pt-offset-y", type=int, default=0,
                    help="Vertical offset in pixels (positive=down, negative=up)")
    ...
    df = df.replace([np.inf, -np.inf], np.nan)      # normalize infinities to NaN
    df = df.where(pd.notna(df), None)               # turn every NaN into Python None
    ...
    ui_cfg = {
        ...
        "pt": {
            ...
            # NEW: placement/positioning from CLI
            "mode": args.pt_mode,           # "flow" | "absolute" | "fixed"
            "anchor": args.pt_anchor,       # CSS selector
            "position": args.pt_position,   # "before" | "after" | "inside"
            "offsetX": args.pt_offset_x,
            "offsetY": args.pt_offset_y
